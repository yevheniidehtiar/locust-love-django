...
from examples_app.models import Author, Book, Category, Product, Customer, Order, OrderItem # Corrected
from examples_app.complex_models import Department, Employee, Project, Task, ProjectAssignment # Corrected, Added ProjectAssignment
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'Successfully created {len(author_list)} Authors.'))

        # Create Books
        self.stdout.write("Creating Books...")
        books_to_create = []
        if not author_list:
            self.stdout.write(self.style.WARNING('No authors found, cannot create books.'))
        else:
            ...
            self.stdout.write(self.style.SUCCESS(f'Successfully created {Book.objects.count()} Books.'))
        ...
        for _ in range(random.randint(5, 10)):
            departments_to_create.append(Department(
                name=self.faker.unique.bs().title() + " Division", # Changed for more uniqueness
        ...
        self.stdout.write(self.style.SUCCESS(f'Successfully created {len(department_list)} Departments.'))

        # Create Employees (from complex_models)
        self.stdout.write("Creating Employees...")
        employees_to_create = []
        if not department_list:
            self.stdout.write(self.style.WARNING('No departments found, cannot create employees.'))
        else:
            for _ in range(random.randint(50, 100)):
            ...
            for emp in employee_list:
                if random.random() < 0.25: # 25% chance to have a manager
                    possible_managers = [m for m in employee_list if m.id != emp.id and m.department == emp.department]
                    if possible_managers:
                        emp.manager = random.choice(possible_managers)
                        emp.save() # Individual save needed for manager assignment
            self.stdout.write(self.style.SUCCESS(f'Successfully created {len(employee_list)} Employees.'))

        # Create Projects (from complex_models)
        self.stdout.write("Creating Projects...")
        projects_to_create = []
        if not department_list:
             self.stdout.write(self.style.WARNING('No departments found, cannot assign projects. They will be null.'))
        for _ in range(random.randint(20, 30)):
            projects_to_create.append(Project(
                name=self.faker.catch_phrase() + " Initiative", # More descriptive names
                code=self.faker.unique.bothify(text='PROJ####??').upper(),
                description=self.faker.text(max_nb_chars=250),
                start_date=self.faker.date_between(start_date="-2y", end_date="-1m"),
                end_date=self.faker.date_between(start_date="today", end_date="+3y"),
                budget=random.randint(20000, 1000000),
                department=random.choice(department_list) if department_list else None
            ))
        ...
        if employee_list and project_list: # Add members to projects if employees exist
            ...
            for proj in project_list:
                ...
                assignment_start_date = proj.start_date if proj.start_date else self.faker.date_this_year()
                members_count = random.randint(1, min(len(employee_list), 7)) # Assign 1-7 members
                ...
                for emp in members_for_project:
                    project_assignments_to_create.append(
                        ProjectAssignment(
                            ...
                            assignment_date=self.faker.date_between(start_date=assignment_start_date, end_date="today"), # Assign date
            ...
            self.stdout.write(self.style.SUCCESS(f'Successfully created {len(project_list)} Projects and assigned members via ProjectAssignment.'))
        else:
            self.stdout.write(self.style.WARNING(f'Skipping Project member assignment due to missing employees or projects.'))
        ...
        if not project_list or not employee_list:
            self.stdout.write(self.style.WARNING('No projects or employees found, cannot create tasks comprehensively.'))
        else:
            for _ in range(random.randint(200, 300)):
                tasks_to_create.append(Task(
                    title=self.faker.sentence(nb_words=random.randint(3,8)), # Use 'title' field
                    description=self.faker.text(max_nb_chars=300),
                    project=random.choice(project_list),
                    assigned_to=random.choice(employee_list), # Use 'assigned_to' field
                    created_by=random.choice(employee_list),
                    due_date=self.faker.date_between(start_date="today", end_date="+1y"),
                    status=random.choice([s[0] for s in Task.STATUS_CHOICES]),
                    priority=random.choice([p[0] for p in Task.PRIORITY_CHOICES]),
                    estimated_hours=random.choice([0.5, 1, 2, 3, 4, 5, 8, 13, 21]) # Fibonacci-ish hours
                ))
            Task.objects.bulk_create(tasks_to_create)
            self.stdout.write(self.style.SUCCESS(f'Successfully created {Task.objects.count()} Tasks.'))
        ...
        for _ in range(random.randint(10, 20)):
            categories_to_create.append(Category(
                name=self.faker.unique.bs().capitalize() + " Category", # Changed for more uniqueness
                description=self.faker.text(max_nb_chars=100)
            ))
        Category.objects.bulk_create(categories_to_create)
        category_list = list(Category.objects.all())
        self.stdout.write(self.style.SUCCESS(f'Successfully created {len(category_list)} Categories.'))

        # Create Products (from examples_app.models)
        self.stdout.write("Creating Products...")
        products_to_create = []
        if not category_list:
            self.stdout.write(self.style.WARNING('No categories found, products will not have categories.'))
        ...
        self.stdout.write(self.style.SUCCESS(f'Successfully created {len(product_list)} Products.'))
        ...
        self.stdout.write(self.style.SUCCESS(f'Successfully created {len(customer_list)} Customers.'))

        # Create Orders (from examples_app.models)
        self.stdout.write("Creating Orders...")
        orders_to_create = []
        if not customer_list:
            self.stdout.write(self.style.WARNING('No customers found, cannot create orders.'))
        else:
            for _ in range(random.randint(200, 300)):
                orders_to_create.append(Order(
                    customer=random.choice(customer_list),
                    order_date=self.faker.date_time_between(start_date="-3y", end_date="now", tzinfo=timezone.get_current_timezone()),
            ...
            self.stdout.write(self.style.SUCCESS(f'Successfully created {len(order_list)} Orders.'))

            # Create OrderItems (from examples_app.models)
            self.stdout.write("Creating OrderItems...")
            order_items_to_create = []
            if not order_list :
                self.stdout.write(self.style.WARNING('No orders found, cannot create order items.'))
            elif not product_list:
                self.stdout.write(self.style.WARNING('No products found, cannot create order items.'))
            else:
                ...
                self.stdout.write(self.style.SUCCESS(f'Successfully created {OrderItem.objects.count()} OrderItems.'))
