...
class IndexedProduct(models.Model):
    ...
    category = models.ForeignKey(Category, related_name="indexed_products", on_delete=models.SET_NULL, null=True, blank=True)
    price = models.DecimalField(max_digits=10, decimal_places=2)
    description = models.TextField(blank=True)
    stock_quantity = models.PositiveIntegerField(default=0)
    created_at = models.DateTimeField(auto_now_add=True)
...
class OrderItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.product and self.price_at_purchase is None: # Set price only if not already set
