from django.db.models import Count  # Added for F821 error
...
def query_caching_example(request):
    ...
    with_cache_results_first, with_cache_time_first, from_cache_first = (
        get_expensive_query_with_cache(min_year, max_year, min_avg_year)
    )

    # Execute the query with caching again (second call - cache hit)
    with_cache_results_second, with_cache_time_second, from_cache_second = (
        get_expensive_query_with_cache(min_year, max_year, min_avg_year)
    )
...
def serializer_optimization_example(request):
    ...
    optimized_serializer = OptimizedAuthorWithBooksSerializer(
        optimized_authors, many=True
    )
