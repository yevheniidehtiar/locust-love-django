            ...
            return render_template(
                ...
                template_args={
                    **environment.web_ui.template_args,
                    ...
                    "extended_tabs": [
                        {"title": "Performance metrics", "key": "performance-metrics"}
                    ],
                    "extended_tables": [
                        {
                            "key": "performance-metrics",
                            "structure": [
                                ...
                                *[
                                    {"key": key, "title": title}
                                    for key, title in PERFORMANCE_METRICS_UI_HEADERS.items()
                                ],
                            ],
                        }
                    ],
                    "extended_csv_files": [
                        {
                            "href": "/performance/csv",
                            "title": "Download Performance Metrics CSV",
                        }
                    ],
                },
            )
...
class UserBehavior(TaskSet):
    ...
    def n_plus_one_example(self):
        with self.client.get(
            ...
            if response.status_code == 200:
                logger.debug(
                    f"n_plus_one_example response status: {response.status_code}"
                )
    ...
    def optimized_query_example(self):
        with self.client.get(
            ...
            if response.status_code == 200:
                logger.debug(
                    f"optimized_query_example response status: {response.status_code}"
                )
    ...
    def expensive_query_example(self):
        with self.client.get(
            ...
            if response.status_code == 200:
                logger.debug(
                    f"expensive_query_example response status: {response.status_code}"
                )
    ...
    def complex_nested_queries_example(self):
        with self.client.get(
            ...
            if response.status_code == 200:
                logger.debug(
                    f"complex_nested_queries_example response status: {response.status_code}"
                )
    ...
    def department_performance_analysis_example(self):
        with self.client.get(
            ...
            if response.status_code == 200:
                logger.debug(
                    f"department_performance_analysis_example response status: {response.status_code}"
                )
